...
from train_multitask_final_serial import MSRSDataset  # 复用Dataset

EXP_NAME = "Exp_MultiTask_SUM_Stage4Only"
# 不确定性图路径 (测试时其实只用原图和Label，但为了Dataset兼容性，可以传路径，代码里不读取即可)
UNCERTAINTY_DIR_TEST = "/home/mmsys/disk/MCL/MultiModal_Project/sam2/data/MSRS/uncertainty_map/test"
...
def test():
    ...
    model.load_state_dict(torch.load(f"checkpoints/{EXP_NAME}/best_model.pth"))
    model.eval()

    dl = DataLoader(MSRSDataset(VAL_DIRS, UNCERTAINTY_DIR_TEST, is_train=False), batch_size=1)
    ...
    with torch.no_grad():
        for v, i, l, _ in tqdm(dl):
            # 不跑 SAM 分支
            out, _, _ = model(v.cuda(), i.cuda(), None)
